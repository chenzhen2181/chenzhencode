...
if (inlet_test)
  %2) enter x and y coordinates of rho points
    ncellsx=332;  dx=5;  %Lm=ncellsx-2  swan=ncellsx-1
    ncellsy=10;  dy=10;    %Mm=ncellsy-2 swan=ncellsy-1 zbath.txt=ncellsx*ncellsy swan x=Lm+1 y=Mm+1
  ...
  %3) set depth 
    depth=load('N:\chenzhen\boshengliu\Visser\bath\Visser_bath_100_10.txt')'
  %4) set masking
    mask_rho=ones(size(depth));
  %5) enter output file name
    fname='N:\chenzhen\boshengliu\Visser\nc\Visser_bath_100.nc';
