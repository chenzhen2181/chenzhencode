...
if (inlet_test)
  %2) enter x and y coordinates of rho points
    ncellsx=277;  dx=100;  %Lm=ncellsx-2  swan=ncellsx-1
    ncellsy=10;  dy=150;    %Mm=ncellsy-2 swan=ncellsy-1 zbath.txt=ncellsx*ncellsy swan x=Lm+1 y=Mm+1
  ...
  %3) set depth 
    depth=load('N:\chenzhen\boshengliu\Visser\bath\Visser_bath_1000_10_adddepth_new.txt')'
  %4) set masking
    mask_rho=ones(size(depth));
  %5) enter output file name
    fname='N:\chenzhen\boshengliu\Visser\nc\Visser_bath_1000_adddepth_new.nc';
