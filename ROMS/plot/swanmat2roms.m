...
% Place a '1' for the files to get, or a '0' to not include them.
 ...
 get_dissip=0;       dissip_file = 'dissip.mat';
 get_rtp=1;          rtp_file    = 'rtp.mat';
 get_tmbot=0;        tmbot_file  = 'tmbot.mat';
 get_ubot=0;         ubot_file   = 'ubot.mat';
 get_wdir=1;         wdir_file   = 'wdir.mat';
 get_wlen=1;         wlen_file   = 'wlen.mat';
 get_break=0;        break_file  = 'qb.mat';
 get_xp=0;           xp_file     = 'xp.mat';
 get_yp=0;           yp_file     = 'yp.mat';
 get_dissip_break=0; dissip_break_file= 'dissip_break.mat';
 get_dissip_wcap=0;  dissip_wcap_file= 'dissip_wcap.mat';
 get_dissip_fric=1;  dissip_fric_file= 'dissip_fric.mat';
 get_Wave_force=0;  wave_force_file= 'force.mat';
...
if(get_Wave_force)
  ID = netcdf.defVar(nc_swan,'Wave_forcex','double',[xrhodimID erhodimID wtdimID]);
  netcdf.putAtt(nc_swan,ID,'long_name','Wave_forcex');
  netcdf.putAtt(nc_swan,ID,'units','Newtons meter-2');
  netcdf.putAtt(nc_swan,ID,'coordinates','xp yp');
  netcdf.putAtt(nc_swan,ID,'field','Wave_forcex, scalar, series');
end
if(get_Wave_force)
  ID = netcdf.defVar(nc_swan,'Wave_forcey','double',[xrhodimID erhodimID wtdimID]);
  netcdf.putAtt(nc_swan,ID,'long_name','Wave_forcey');
  netcdf.putAtt(nc_swan,ID,'units','Newtons meter-2');
  netcdf.putAtt(nc_swan,ID,'coordinates','xp yp');
  netcdf.putAtt(nc_swan,ID,'field','Wave_forcey, scalar, series');
end
if(get_dissip_break)
  ID = netcdf.defVar(nc_swan,'dissip_break','double',[xrhodimID erhodimID wtdimID]);
  netcdf.putAtt(nc_swan,ID,'long_name','dissip_break');
  netcdf.putAtt(nc_swan,ID,'units','Newtons meter-2');
  netcdf.putAtt(nc_swan,ID,'coordinates','xp yp');
  netcdf.putAtt(nc_swan,ID,'field','dissip_break, scalar, series');
end
if(get_dissip_wcap)
  ID = netcdf.defVar(nc_swan,'dissip_wcap','double',[xrhodimID erhodimID wtdimID]);
  netcdf.putAtt(nc_swan,ID,'long_name','dissip_wcap');
  netcdf.putAtt(nc_swan,ID,'units','Newtons meter-2');
  netcdf.putAtt(nc_swan,ID,'coordinates','xp yp');
  netcdf.putAtt(nc_swan,ID,'field','dissip_wcap, scalar, series');
end
if(get_dissip_fric)
  ID = netcdf.defVar(nc_swan,'dissip_fric','double',[xrhodimID erhodimID wtdimID]);
  netcdf.putAtt(nc_swan,ID,'long_name','dissip_fric');
  netcdf.putAtt(nc_swan,ID,'units','Newtons meter-2');
  netcdf.putAtt(nc_swan,ID,'coordinates','xp yp');
  netcdf.putAtt(nc_swan,ID,'field','dissip_fric, scalar, series');
end
...
if( get_Wave_force)
   disp('   Wave_force -> Wave_force')
   F=load(wave_force_file);
   vname=fieldnames(F);
   wavex=vname(1);
   wavey=vname(2);
   for K=1:length(wavex)
       disp(sprintf('%g/%g Hwave saved',K,length(wavex)))
       WForce_x=getfield(F,char(wavex(K)));
       WForce_x(find(WForce_x == -999)) = 0.0;   % flag
       WForce_x(find(isnan(WForce_x)))=0.0;     % land
       ncwrite(fname,'Wave_forcex',squeeze(WForce_x).',[1 1 K])
       clear WForce_x;
   end
   for K=1:length(vname(2))
       disp(sprintf('%g/%g Hwave saved',K,length(wavey)))
       WForce_y=getfield(F,char(wavey(K)));
       WForce_y(find(WForce_y == -999)) = 0.0;   % flag
       WForce_y(find(isnan(WForce_y)))=0.0;     % land
       ncwrite(fname,'Wave_forcey',squeeze(WForce_y).',[1 1 K])
       clear WForce_y;
   end
   clear F vname K wavex wavey
end
